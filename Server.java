                ...
                while (true) {
                    try {
                        ...
                        if (e.getMessage().equals("OK")) {
                            d.writeObject("OK");
                            d.flush();    
                        }if (e.getMessage().equals("ERREUR DE CONNECTION")) {
                            d.writeObject("ERREUR DE CONNECTION");
                            d.flush();    
                        }if (e.getMessage().equals("CETTE BASE N'EXISTE PAS")) {
                            d.writeObject("BASE INEXISTANTE");
                            d.flush();    
                        }if (e.getMessage().equals("CETTE TABLE N'EXISTE PAS")) {
                            d.writeObject("TABLE INEXISTANTE");
                            d.flush();    
                        }if (e.getMessage().equals("DATABASE EXISTANT")) {
                            d.writeObject("DATABASE EXISTANT");
                            d.flush();    
                        }if (e.getMessage().equals("TABLE EXISTANT")) {
                            d.writeObject("TABLE EXISTANT");
                            d.flush();    
                        }if (e.getMessage().equals("TSY AMPY ILAY DATA")) {
                        ...
                        if (e.getMessage().equals("DISCONNECTED")) {
                            c=(String[])di.readObject();
                            bdd=new Connect(c[0],c[1]);
                        }
